...
public class BST<E> {
    ...
    // Trying again
    // Inspired by Mr. Bakker's code
    private Node<E> delete(int key, Node<E> node) {
        if (node != null) {
            if (node.key == key) {
                if (node.left != null || node.right != null) {
                    if (node.left == null) {
                        Node<E> min = getMinDelete(node.right);
                        node.key = min.key;
                        node.val = min.val;
                        node.right = delete(min.key, node.right);
                    } else {
                        Node<E> max = getMaxDelete(node.left);
                        node.key = max.key;
                        node.val = max.val;
                        node.left = delete(max.key, node.left);
                    }
                } else {
                    node = null;
                }
    ...
    private void put(int key, E val, Node<E> node) {
        if (node == null) {
